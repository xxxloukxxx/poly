...
\begin{ex}
	...
	\begin{enumerate}
		...
		\item \textbf{Retrouver l'effectif total :} Dans un club sportif, $42$ adhérents joue au tennis, ce qui représente $35\,\%$ de l'effectif du club ($p = 0{,}35$).
...
\begin{df}
	...
	Pour l'exprimer en \textbf{pourcentage}, on calcule : \cmath{t_{\,\%} = \dfrac{V_F - V_I}{V_I} \times 100}
\end{df}
	...
	\begin{wrapstuff}[r,width=6cm]
		\begin{center}
			\begin{tikzpicture}[x=0.75cm, y=1cm, >=Stealth]
				...
				\node (VI) at (0,0) {$80\text{~\euro{}}$};
				\node (VF) at (5,0) {$92\text{~\euro{}}$};
				\draw[->, thick] (VI) -- (T);
				\draw[->, thick] (T) -- (VF);
...
\begin{rmq}
	...
	\begin{itemize}
		\item \makebox[4cm][l]{$t = \dfrac{V_F - V_I}{V_I} \times 100$} pour calculer le taux d'évolution en pourcentage
	...
	\begin{wrapstuff}[r, width=7cm]
		\begin{center}
			...
			\begin{tikzpicture}[x=1cm, y=1cm, >=Stealth]
				...
				\node[draw=black!80, fill=black!5, thick, rounded corners=4pt, inner sep=5pt, minimum width=1cm, align=center] (TR) at (4.2,0) {$t_{R}$};
				\draw[->, thick] (VI1) -- (T);
				\draw[->, thick] (T) -- (VF);
				\draw[->, thick] (VF) -- (TR);
				\draw[->, thick] (TR) -- (VI2);
...
\begin{exemple}
	...
	\begin{enumerate}
		\item {Calcul du coefficient multiplicateur initial :} $$CM = 1 + \frac{20}{100} = 1{,}20$$
		\item {Calcul du coefficient multiplicateur réciproque :} $$CM_R = \frac{1}{CM} = \frac{1}{1{,}20} = \frac{5}{6} \approx 0{,}8333$$
		\item {Calcul du taux d'évolution réciproque :} $$t_R = (CM_R - 1) \times 100 = \left(\frac{5}{6} - 1\right) \times 100 = -\frac{100}{6} \approx -16{,}67\,\%$$
	\end{enumerate}
...
\begin{exercice}
	...
	\begin{enumerate}
		...
		\item Calculer le taux d'évolution global $t_g$ de la population de licornes sur cette période.\\ La population a-t-elle augmenté ou diminué au global ?
